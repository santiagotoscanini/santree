...
├── lib/
│   ├── ai.ts            # Shared AI logic (context resolution, prompt rendering); launch/run/binary delegate to lib/agents/
│   ├── agents/          # Pluggable AI agent backend (Claude Code / Codex); getAiAgent() factory + capability flags
...
`resolveAIContext()` finds repo, branch, and issue identifier, then fetches the issue from the active tracker (Linear or GitHub — see [Issue tracker abstraction](#issue-tracker-abstraction-libtrackers)). It returns `AIContext` with the issue plus `trackerName`/`issueNoun` so prompt templates and downstream code don't hardcode a vendor name. `renderAIPrompt()` renders a named Nunjucks template with context. `launchAgent()`/`runAgent()`/`runAgentAsync()` are thin wrappers over `getAiAgent()` (see [AI agent abstraction](#ai-agent-abstraction-libagents)) — they spawn the active agent's CLI (Claude or Codex), not Claude specifically. `fetchAndRenderPR(branch)` and `fetchAndRenderDiff(branch)` pre-fetch structured PR feedback and diff data for injection into prompts. `fetchAndRenderFixContext(branch)` renders the compact per-iteration brief the auto-fix loop consumes — conflict status (via `getPRMergeStateAsync`), failing checks tagged fixable/manual (via `classifyCheck`), and the 👍-approved review threads (via `getPRReviewThreadsAsync` + `getApprovedReviewThreads`), all in `lib/github.ts`. `computeFixContext(branch, santreeCmd)` returns the same brief plus the structured `directive` and a state `signature` — used by the santree-driven Codex fix loop to decide continue/wait/stop and detect being stuck.

### AI agent abstraction (`lib/agents/`)

Santree drives a pluggable AI coding agent — **Claude Code** (default) or **OpenAI Codex** — the same way it abstracts trackers and multiplexers. `getAiAgent()` (`lib/agents/index.js`) resolves the active adapter: `SANTREE_AGENT` env (`claude`|`codex`) → `agent` in `~/.config/santree/config.json` (set from `santree config` → System → **AI agent**) → default `claude`. Selection is **global** (which CLI you have installed), not per-repo, so it lives in `config-store.ts`, not metadata.json.

The `AiAgent` interface (`lib/agents/types.ts`) exposes `resolveBinary()`, `getInstalledVersion()`, `launchInteractive()`, `runHeadless()`/`runHeadlessAsync()`, `buildResumeCommand()`, `buildLaunchLine()`, plus `displayName`/`installPackage`/`installHint`, and — the part the rest of the app gates on — **capability flags**: `canPresetSessionId`, `supportsStatusline`, `supportsRemoteControl`, `supportsReadableTodos`, `supportsSelfPacedLoop`, `supportsSlashSkills`, and the optional `readTodos?`/`findSessionCwd?` methods (feature-detected like `IssueTracker.canMutate`). **Never branch on `kind === "codex"` outside the factory / diagnostics strings — gate on a flag.** The Claude adapter (`agents/claude/`) owns `resolveClaudeBinary()` (cmux-bundled path) and maps to `--permission-mode`/`--session-id`/`--resume`/`-p --allowedTools`; the Codex adapter (`agents/codex/`) maps to `--sandbox`/`--ask-for-approval`/`codex exec -o`/`resume`. Capability flags drive the adaptive surfaces: Codex (all the Claude-only flags false) drops the statusline + remote-control rows from `santree config`, hides the dashboard Tasks section, skips the session-id store in `worktree/work.tsx`, and takes the santree-driven fix-loop path. The Codex adapter's exact flag spellings are documented-but-unverified (the spike couldn't run without `codex` installed) — they're commented as such and isolated to that one file.

### Auto-fix loop (`pr fix`)

`santree pr fix` is **one flow** with two drivers, chosen by `getAiAgent().supportsSelfPacedLoop`:

- **Claude (self-paced):** a **self-driving Claude Code `/loop`** (`prompts/fix-loop.njk`) that handles CI failures *and* review comments in the same loop. **Critical: invoke `/loop` WITHOUT an interval** (`/loop <prompt>`, not `/loop 5m …`). No-interval `/loop` is the *self-paced* mode (driven by `ScheduleWakeup`) that keeps context across iterations and genuinely stops on our conditions; passing an interval picks *cron mode* (`CronCreate`, fire-and-forget, fresh context each firing, won't self-terminate) — wrong for a fix loop.
- **Codex (santree-driven):** Codex has no self-scheduling primitive, so `pr fix` (`commands/pr/fix.tsx`) owns the loop itself — a bounded `while` in the command that each iteration calls `computeFixContext` (the same stateless brain), acts on its `directive` (run the agent headless to merge/fix → push/resolve, or `wait` for CI, or `stop`), heartbeats the `.santree/fix-loops/` marker, and applies the "same actionable `signature` twice running → stuck" judgment the stateless brief can't track. The marker/badge plumbing is unchanged.

There's no separate one-shot "fix comments" mode (the bare one-shot, and the old `--loop` flag that selected it, were both removed; the loop is the only flow).
...
| Variable | Effect |
|---|---|
| `SANTREE_AGENT` | Override the active AI agent backend for a single invocation: `claude` or `codex`. Takes precedence over the `agent` key in `~/.config/santree/config.json` (set from `santree config` → System → **AI agent**). If unset, defaults to `claude`. Resolved via `getConfiguredAgent()` / `getAiAgent()`. |
...
On Claude, santree launches with `--permission-mode auto` for implement runs and `--permission-mode plan` for plan-mode runs (`santree worktree work --plan`). Auto-acceptance of non-mutating tools while planning is governed by Claude Code's `useAutoModeDuringPlan` setting in `~/.claude/settings.json`, not by santree. There is no opt-in env var — worktree-scoped automation is the default. Set `--permission-mode default` upstream if you ever need stricter prompting. On Codex, the equivalent is `--sandbox` + `--ask-for-approval` (`workspace-write`/`never` for implement, `read-only` for plan-mode and read-only headless calls) — see [AI agent abstraction](#ai-agent-abstraction-libagents).

## External Dependencies

Required: Node.js >= 20, Git, GitHub CLI (`gh`), and an AI agent CLI — Claude Code (`claude`, `@anthropic-ai/claude-code`, default) or Codex (`codex`, `@openai/codex`); pick one in `santree config` → System → **AI agent** (see [AI agent abstraction](#ai-agent-abstraction-libagents))
Optional:
